...
def table_str_bin(result_set, row_labels, labels_long=None, binary=False):
    if binary:
        ...
        # 将PEHE和Bias_ATE放在最前面,便于查看主要评估指标
        cols = ['pehe', 'bias_ate', 'rmse_fact', 'rmse_cfact', 'rmse_ite', 'ate_pred', 'ate', 'att_pred', 'att', 'bias_att', 'atc_pred', 'atc', 'bias_atc', 'objective', 'pehe_nn', 'auc']
...
def select_parameters(results, configs, stop_set, stop_criterion, choice_set, choice_criterion):
    ...
    # print(results[stop_set][stop_criterion].shape)
    ...
    print ('average:', np.mean(i_sel,1))
    print ('the length:', [results[stop_set][stop_criterion].shape[2]])
...
def plot_cfr_evaluation_bin(results, configs, output_dir):
    ...
    print ('Early stopping:')
    print('average:', np.mean(i_sel, 1))
    print('the length:', [results[EARLY_STOP_SET_BIN][EARLY_STOP_CRITERION_BIN].shape[2]])
    ...
    for i in range(len(alphas)):
        ...
        if i==ia:
            m = ' *'
        method_label = configs[0].get('imb_fun', 'repr-ortho')
        labels.append('CFR-%d-%d %s a=%.2g%s' % (di,do,method_label,alphas[i],m))
